...
nonisolated func floatArrayToCGImage(data: UnsafePointer<Float>, width: Int, height: Int, flipY: Bool = true) -> CGImage? {
    let bytesPerPixel = 4
...
nonisolated func cgImageToFloatArray(_ image: CGImage, width: Int, height: Int, flipY: Bool = true) -> [Float]? {
    let bytesPerPixel = 4
    let byteCount = width * height * bytesPerPixel
...
nonisolated func bytesToCVPixelBuffer(data: UnsafePointer<UInt8>, width: Int, height: Int, flipY: Bool = true) -> CVPixelBuffer? {
...
nonisolated func floatArrayToCVPixelBuffer(data: UnsafePointer<Float>, width: Int, height: Int, flipY: Bool = true) -> CVPixelBuffer? {
    var pixelBuffer: CVPixelBuffer?
    let status = CVPixelBufferCreate(kCFAllocatorDefault, width, height, kCVPixelFormatType_32BGRA, nil, &pixelBuffer)
...
nonisolated func saveCGImageToDisk(cgImage: CGImage, filename: String) {
...
nonisolated func saveImage(mask: [Float], width: Int, height: Int, filename: String, grayscale: Bool = true) -> String {
...
nonisolated func grayscaleToRGB(mask: [Float], width: Int, height: Int) -> [Float] {
...
nonisolated func flipYAxis(image: [Float], width: Int, height: Int) -> [Float] {
...
nonisolated func getCurrentTimestamp() -> UInt64 {
